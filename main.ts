...
async function getSigningKey(did: string, forceRefresh: boolean): Promise<string> {
...
async function getEncodedKeypair(did: string): Promise<{ publicKey: string; secretKey: string }> {
  const keypair = await getKeypair(did);
  return {
...
if (!serviceDid)
  throw new Error('Must set DID environment variable to the DID of this deployed service.');

// Return the service DID
router.get('/.well-known/did.json', ({ url }) => ({
  '@context': ['https://www.w3.org/ns/did/v1'],
  id: `did:web:${new URL(url).host}`,
  service: [
    {
      id: '#pigeon_keyserver',
...
router.get('/xrpc/public.key.pigeon.muni.town', async ({ query }) => {
  const { did } = query;
  if (typeof did !== 'string' || !did) return error(400, 'DID query parameter required');
  return {
...
router.all('*', async (ctx) => {
  ...
  if (!authorization.startsWith('Bearer ')) return error(403, 'Bearer token required');
...
router.get('/xrpc/key.pigeon.muni.town', ({ did }: Ctx) => getEncodedKeypair(did));
